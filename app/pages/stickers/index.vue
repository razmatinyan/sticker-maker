	...
	<div class="space-y-5 max-w-6xl">
		...
		>
			...
			<div class="flex items-center gap-2">
				...
				<NuxtLink to="/editor">
					<Button size="sm" class="gradient-btn rounded-lg text-sm">
		...
		<!-- Loading -->
		<div v-if="isLoading" class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 gap-3">
			<Skeleton
				v-for="i in 12"
				:key="i"
		...
		>
			...
			<div>
				<p class="font-semibold">No stickers yet</p>
				<p class="text-sm text-muted-foreground mt-1">
					Create your first sticker in the editor
				</p>
			</div>
			<NuxtLink to="/editor">
				<Button class="gradient-btn rounded-lg px-4 py-2 text-sm">
		...
		>
			<div
				...
				:class="{
					'ring-2 ring-accent-violet': selected.has(sticker.id),
			...
			>
				...
				<!-- Hover overlay -->
				<div
					class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-150 flex items-center justify-center gap-2"
				>
					<button
