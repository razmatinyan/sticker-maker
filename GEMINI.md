...
The design uses a **Soft Glassmorphic Dark** aesthetic — warm layered dark surfaces with Indigo-Violet gradient accents, atmospheric floating orbs, and real glassmorphism (visible blur + multi-layer shadows).

### Fonts

- **Display headings**: Plus Jakarta Sans (semibold/bold, tracked tight)
...
```css
.glass          /* Frosted glass surface: 6% white, blur 20px, saturate 1.4 */
.glass-card     /* Glass + indigo ring shadow + rounded + hover lift */
.glass-surface  /* Non-interactive glass (no hover transform) */
.gradient-text  /* Gradient text fill: indigo → violet */
.gradient-btn   /* Gradient background button: indigo → violet */
.glow-orb       /* Animated floating gradient orb for background atmosphere */
.noise-overlay  /* Subtle noise texture at 2.5% opacity */
.stagger-item   /* Staggered entrance animation (set --stagger-index) */
.tracking-display   /* -0.04em letter spacing for hero text */
.tracking-heading   /* -0.025em letter spacing for headings */
```

### Color Reference

```
Background:         hsl(240 15% 4%)    #09090f  (deep blue-black)
Foreground text:    hsl(0 0% 97%)      #f7f7f7
Muted text:         hsl(240 5% 55%)    (warmer gray)
Accent Indigo:      hsl(239 84% 67%)   #6366f1
Accent Violet:      hsl(258 90% 66%)   #8b5cf6
Accent Rose:        hsl(350 89% 60%)   #f43f5e
Card surface:       hsl(240 10% 7%)    #101018
Border:             hsl(240 6% 15%)
```
